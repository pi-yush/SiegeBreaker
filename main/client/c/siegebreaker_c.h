...
#define SSL_PORT 443
uint8_t key[32] = "**SiegebreakerDemo**************";
int timeout=1;
int use_pub_priv=1;
  ...
  unsigned char *iv, unsigned char *plaintext)
{
  ...
  if(!(ctx = EVP_CIPHER_CTX_new())) { handleErrors(); return -1; }
   ...
   * this stage.
   */
  if(1 != EVP_DecryptFinal_ex(ctx, plaintext + len, &len)) { handleErrors(); return -1; }
  plaintext_len += len;
       ...
       int source, char *self_ip, char *target_ip, int rst)
{
  ...
  tcph->syn=0;
  tcph->urg=0;
  tcph->check = tcp_sum_calc (tot_len, parseIPV4string(self_ip), parseIPV4string(target_ip),
      (unsigned char *) tcph);
  return 0;
}
...
Sniff_response * callback(char *pkt, int pkt_len, char *self_ip, char *target_ip) {
  ...
  if (strcmp(src_addr, "127.0.0.1") == 0 || strcmp(dst_addr, "127.0.0.1") == 0) {
    return NULL;
  }
  // eth0
  // avoid packets not meant for us

  if (strcmp(src_addr, self_ip) != 0 && strcmp(dst_addr, self_ip) != 0)
      return NULL;
 ...
 if (target_ip != NULL) {
  ...
  tcph = (struct tcphdr *) ((unsigned  char *) pkt + sizeof(struct iphdr));

  // one of the port should be 443
  ...
  char *ssl_pkt = pkt + sizeof(struct iphdr) + sizeof(struct tcphdr) + 12;
  // now we have TCP psh packets with source port 443, these are most probably SSL packets
  //printf("%x %x %x\n", *ssl_pkt, *(ssl_pkt + 1), *(ssl_pkt + 2));
  sslhdr = malloc(sizeof (SSL_data_hdr));
  if (ssl_pkt[0] != 23) {
   //if (DEBUG) printf("Hacking to make everyone happy! Tcp options??\n");
   ssl_pkt = ssl_pkt - 12;
  }
  ...
  //if (ssl_pkt + 5 + sslhdr->length <= pkt + pkt_len) { // verify if its a valid SSL packet
  //  return NULL;
  //}
...
Sniff_response* decode_salsa(Sniff_response *s) {
  ...
  memcpy(priv,s->msg,256);
	...
	memcpy(nonce, s->msg, 16);
  	...
  	plaintext_len = decrypt(some, s->length - 16, key, nonce, plain);
  	if (plaintext_len == -1) {
        	free(some);
		return s;
  	}	 
...
Sniff_response* sniff(int sock_raw, char *self_ip, char *target_ip, int decode_content) {
  ...
  while (1) {
    ...
    data_size = recvfrom(sock_raw, buf, 65536, 0, &saddr, &saddr_size);
    if (data_size < 0)
    {
      printf("Failed to read packet from raw socket!");
      free(buf);
      return NULL;
    }
    msg = callback(buf, data_size, self_ip, target_ip);
    if (msg != NULL) {
      if (decode_content)
     	 msg = decode_salsa(msg);
      free(buf);
      return msg;
    }
  }
...
int send_tls_packet(Sniff_response *resp, char *content, int sock_raw, char *tip, int length) {
  if (DEBUG) printf("Sending pak seq=%ld ack=%ld content-len: %d, tip: %s!\n", resp->reply_seq, resp->reply_ack, length, tip);
  ...
  if(use_pub_priv)
  {
	...
	//printf(" content enc : %s \n len : %d",encrypted,enc_content);
 	...
 	enc_content = encrypt(content, length, key, nonce, encrypted);
  //printf("%d:%d\n", length, enc_content);
  int content_len = enc_content;
  unsigned short tls_length = htons(16 + content_len);
  int size_tls_header = 5; // first byte for type of TLS packet(TLS Data), next two bytes for TLS Version, the next 2 bytes for TLS content length

  int pkt_size = sizeof(struct tcphdr) + ntohs(tls_length) + size_tls_header;
  ...
  create_tcp_packet (pkt_size, tcpip_pkt, resp->urg_ptr,
     ...
     resp->reply_ack, SSL_PORT,
     resp->port, resp->ip, tip, 0);
 ...
 //printf("\n");
  ...
  if(use_pub_priv)
  {
	memcpy(tls_start + 5,encrypted,content_len);
	use_pub_priv=0;
  }
  else
  {
  	memcpy(tls_start + 5, nonce, 16);
